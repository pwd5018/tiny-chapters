...
- Phase 21 is complete for the current mobile/service boundary: `memory_provider_grants` and `memory_provider_access_logs` provide owner-scoped authorization and audit storage; `retrieveMemoriesForProvider` requires an active `memories:read` grant, forces finalized-only retrieval, logs summarized access, and validates `/memory/:id` deep links. External/server-side transport is deferred to Phase 22.
